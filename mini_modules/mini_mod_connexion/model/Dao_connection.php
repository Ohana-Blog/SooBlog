...
class Dao_connection extends DaoConnect{
    ...
    public function connexion(User $user){
        ...
        try {
                $select = $this->bd->prepare("SELECT mail,id_u,pseudo,url FROM user WHERE mail = ? AND mdp = ?");
...
//                $data = $select->fetch(PDO::FETCH_ASSOC);
        ...
        if(($donnees = $select->fetch(PDO::FETCH_ASSOC)) === FALSE){
            throw new Dao_connection_Exeption("Pas de données L'utilisateur de saisie");
        }
